...
for label in valid_classes:
    ...
    # Desired baseline target
    target = aug_target

    # Apply augmentation cap:
    # final_count <= n_real * 3  → augmented <= n_real * 2
    max_allowed = n_real * 3

    # Compute the TRUE target after cap
    true_target = min(target, max_allowed)

    if n_real >= true_target:
        print(f"  Label {label:2d} ({class_name:25s}): {n_real} >= {true_target}, no augmentation")
        continue

    n_to_add = true_target - n_real

    print(f"  Label {label:2d} ({class_name:25s}): {n_real} → {true_target} (+{n_to_add} augmented)")
