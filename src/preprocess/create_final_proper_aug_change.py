...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "processed_data/final/memory_safe/own_nonVPN_p2p_2/more_aug")
...
def augment_single_image(img: np.ndarray) -> np.ndarray:
    """
    More realistic augmentation for traffic images.

    img: (28, 28), uint8
    """
    aug = img.astype(np.float32)

    # 1) small Gaussian noise (60% of the time)
    if np.random.rand() < 0.6:
        noise = np.random.normal(0.0, 3.0, aug.shape)
        aug = aug + noise

    # 2) small horizontal shift (40% of the time)
    ...
    # 3) small vertical shift (30% of the time)
    if np.random.rand() < 0.3:
        vshift = np.random.randint(-2, 3)
        aug = np.roll(aug, vshift, axis=0)
        if vshift > 0:
            aug[:vshift, :] = 0.0
        elif vshift < 0:
            aug[vshift:, :] = 0.0

    # 4) random erasing: small block drop (30% of the time)
    if np.random.rand() < 0.3:
        h, w = aug.shape
        eh = np.random.randint(2, 5)
        ew = np.random.randint(4, 9)
        y = np.random.randint(0, h - eh)
        x = np.random.randint(0, w - ew)
        aug[y:y+eh, x:x+ew] = 0.0

    # 5) slight contrast / intensity scaling (50% of the time)
...
def make_augmented_sample(class_images: np.ndarray) -> np.ndarray:
    """
    Optionally do 'same-class mixup' before local augment:
      - pick 1 or 2 base images from same class
      - blend them a bit, then apply pixel-level aug
    """
    if len(class_images) == 1 or np.random.rand() < 0.5:
        # simple augmentation on one image
        base = class_images[np.random.randint(0, len(class_images))]
        mixed = base
    else:
        # mixup between two images of the same class
        idx1, idx2 = np.random.randint(0, len(class_images), size=2)
        img1 = class_images[idx1].astype(np.float32)
        img2 = class_images[idx2].astype(np.float32)
        lam = np.random.uniform(0.4, 0.7)  # mix ratio
        mixed = lam * img1 + (1.0 - lam) * img2
        mixed = np.clip(mixed, 0, 255).astype(np.uint8)

    return augment_single_image(mixed)


# ----------------- adaptive target around ~5k -----------------
train_counter = Counter(y_train)
print("  Original train distribution:", dict(train_counter))
...
aug_target = max_real
print(f"  Adaptive augmentation target (per class): {aug_target}")
...
for label in valid_classes:
    ...
    if n_in_train >= aug_target:
        print(f"  Label {label:2d} ({class_name:25s}): {n_in_train} >= {aug_target}, no aug needed")
        continue

    # Get all real training images for this class
    class_mask = y_train == label
    class_images = X_train[class_mask]

    if len(class_images) < 50:
        print(f"  [!] Label {label} ({class_name}) has only {len(class_images)} real train samples. "
              f"Augmenting heavily up to {aug_target} may be risky / synthetic heavy.")

    n_to_add = aug_target - n_in_train
    print(f"  Label {label:2d} ({class_name:25s}): {n_in_train} → {aug_target} (+{n_to_add} augmented)")

    for _ in range(n_to_add):
        aug_img = make_augmented_sample(class_images)
