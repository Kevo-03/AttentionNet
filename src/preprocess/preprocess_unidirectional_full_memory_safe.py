...
IDX_DIR = os.path.join(PROJECT_ROOT, "processed_data/memory_safe_unidirectional_full")
MAX_LEN = 784  # 28x28
ROWS, COLS = 28, 28

DATA_FILENAME = "data_unidirectional_full.npy"
LABELS_FILENAME = "labels_unidirectional_full.npy"

UNIDIRECTIONAL = True
USE_L7_PAYLOAD_ONLY = False
...
def _application_payload_bytes(pkt):
    if TCP in pkt:
...
def _packet_bytes(pkt):
    if USE_L7_PAYLOAD_ONLY:
        return _application_payload_bytes(pkt)
    return _anonymized_packet_bytes(pkt)
...
def _merge_batches(num_batches):
    ...
    data_path = os.path.join(IDX_DIR, DATA_FILENAME)
    labels_path = os.path.join(IDX_DIR, LABELS_FILENAME)
...
def visualize_samples(samples_per_label=16):
    ...
    data_file = os.path.join(IDX_DIR, DATA_FILENAME)
    labels_file = os.path.join(IDX_DIR, LABELS_FILENAME)
